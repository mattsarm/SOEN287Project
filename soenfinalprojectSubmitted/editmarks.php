...
<?php
if(isset($_GET['editid']) and isset($_GET['qnum'])){
$id = $_GET['editid'];
$qnum = $_GET['qnum'];
$sql = "SELECT questionWeight FROM marks WHERE assessmentId=$id AND questionNumber=$qnum";
$result= mysqli_query($conn,$sql);
if($result){
$row = mysqli_fetch_assoc($result);
$val = $row['questionWeight'];    
}
}

if(isset($_POST['submit'])){
    $weight = $_POST['weight'];
    $sql = "UPDATE marks SET questionWeight=$weight WHERE assessmentId=$id AND questionNumber=$qnum";
    $result= mysqli_query($conn,$sql);
    if($result){
...
<body>
    ...
    <section class="vh-100" id="background-color">
        ...
        <form method="POST">
            ...
            <input type="number" class="form-control" id="weight" name="weight" value=<?php echo "$val"?> placeholder="Weight" aria-describedby="weightHelp">
